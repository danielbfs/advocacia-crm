...
class MessagingService:
    async def get_active_conversations(self, db: AsyncSession, channel: str | None = None):
        """Get all active conversations from both Clients and Leads."""
        from app.modules.leads.ai_models import LeadConversation

        # 1. Fetch Client Conversations
        q_client = select(Conversation).where(Conversation.status == "active")
        if channel:
            q_client = q_client.where(Conversation.channel == channel)
        res_client = await db.execute(q_client)
        clients = list(res_client.scalars().all())
    ...
    async def toggle_control(self, db: AsyncSession, conversation_id: uuid.UUID, control: str):
        """Switch control between 'ai' and 'human' for either Client or Lead conversation."""
        if control not in ("ai", "human"):
            raise ValueError("Control must be either 'ai' or 'human'")

        # Try Client Conversation
    ...
    async def get_messages(self, db: AsyncSession, conversation_id: uuid.UUID):
        """Get all messages for a conversation (Client or Lead)."""
        # Try Client Messages
        res = await db.execute(
            select(Message)
    ...
    async def send_human_message(self, db: AsyncSession, conversation_id: uuid.UUID, text: str, channel: str, chat_id: str):
        """Send a message on behalf of a human and record it in the DB (Client or Lead)."""
        ...
        # Record message in Client table
